...
var ExBuffer = function (bufferLength) {
	var self = this;
    events.call(this);//继承事件类
    var _headLen = 4; //msgheader里的size大小，2或4个字节（含size本身大小字节数）
    var _endian = 'B';
        ...
        while(true){
            ...
            if(_dlen == 0){
                ...
                if(_buffer.length - _readOffset >= _headLen){
                    ...
                    console.log('----------:readUInt8(0):' + _buffer.readUInt8(0));
                    console.log('----------:readUInt8(1):' + _buffer.readUInt8(1));
                    console.log('----------:readUInt8(2):' + _buffer.readUInt8(2));
                    console.log('----------:readUInt8(3):' + _buffer.readUInt8(3));
                }else {//
            ...
            console.log('onReceivePackData--------getLen:' + getLen());
            console.log('onReceivePackData--------_dlen:' + _dlen);
            ...
            if(getLen() >= _dlen){
                ...
                console.log('onReceivePackData--------kkk:' + dbuff.length);
                try {
                    _dlen = 0;
...
util.inherits(ExBuffer, events);//继承事件类
